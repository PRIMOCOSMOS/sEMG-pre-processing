...
\subsubsection{Adaptive Thresholding with Improved Parameters}

The detection threshold adapts to signal characteristics:
\begin{equation}
\theta_{adaptive} = \mu_E + \alpha \cdot \sigma_E \cdot s
\end{equation}
where $\mu_E$ and $\sigma_E$ are the mean and standard deviation of the energy profile, $\alpha$ is the adaptive threshold factor (default: 0.5), and $s$ is the user-adjustable sensitivity parameter.

\textbf{Improved Default Parameters:}
\begin{itemize}
    \item Energy threshold percentile: 0.4 (reduced from 0.65 for less strict detection)
    \item Temporal compactness: 0.15 (reduced from 0.3 to capture segments containing peaks, not just peaks themselves)
\end{itemize}

The lower thresholds ensure that detected segments span the duration of muscle actions (typically several seconds), rather than only capturing instantaneous peak moments.
